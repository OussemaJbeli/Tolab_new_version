            ...
            'auth' => function () use ($request) {
                return [
                    'user' => $request->user() ? [
                        ...
                        'etudient_level' => $request->user()->etudient_level,
                        'Master_level' => $request->user()->Master_level,
                    ...
                    'chanel' => function () use ($request) {
                        $id_chanel = session('selected_channel_id', null);
                    
                        if ($request->user() && $id_chanel) {
                            return chanels::where('chanels.id', $id_chanel)
                    ...
                    'links' => function () use ($request) {
                        $id_chanel = session('selected_channel_id', null);
                    
                        if ($request->user() && $id_chanel) {
                            return Links::where('id_chanel', $id_chanel)
                            ->get();
                        }
                        return null;
                    },  
